...
<%@ page import="com.example.practice.service.FavoriteService" %>
<%@ page import="com.example.practice.entity.FavoriteUrl" %>
<%@ page import="java.util.Objects" %>
<%@ page import="com.example.practice.service.AccountService" %>
...
<head>
    ...
    <%
        ...
        String accountIdCookie = null;
        if(cookies != null){
            for(int i=0; i < cookies.length; i++){
                Cookie cookie = cookies[i];
                if(Objects.equals(cookie.getName(), "accountId")) accountIdCookie = cookie.getValue() ;
            }
        }
    %>
    ...
    <script>
        ...
        function change(favoriteName,favoriteUrl){
            ...
            if(btn.value === '☆ 즐겨찾기'){
                ...
                $.ajax({
                    url : "/favorite/create",
                    type: "get",
                    data: {"username" : "<%=accountIdCookie%>", "favoriteUrl" : favoriteUrl}
                });
            }else{
                btn.value = '☆ 즐겨찾기';
                $.ajax({
    ...
    <ul id="nav">
        ...
        <li>
            <%if(accountIdCookie!=null){ %>
                <a class="Part" href="/logout"><%=accountIdCookie%></a>
            <% } else { %>
        ...
        <li>
            <%if(accountIdCookie!=null){ %>
                <a class="Part" href="/favorite">즐겨찾기</a>
            <% } %>
        </li>
    </ul>
</header>
        ...
        <c:forEach var="url" items="${urls}" varStatus="status">
            <c:if test="${title!=''}">
                <tr>
                    ...
                    ${time[status.index]} &nbsp;
                    <% System.out.println(accountIdCookie);
                        if(accountIdCookie!=null){%>
                            <c:if test="${findDb[status.index]=='1'}">
                                <input type='button' id="${id[status.index]}" value="★ 즐겨찾기" onclick="change('${id[status.index]}','${url}');"/>
                            </c:if>
                            <c:if test="${findDb[status.index]=='0'}">
                                <input type='button' id="${id[status.index]}" value="☆ 즐겨찾기" onclick="change('${id[status.index]}','${url}');"/>
                            </c:if>
                    <% } %>
                    <br>
